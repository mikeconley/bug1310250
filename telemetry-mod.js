(function (exports) {
  ...
  Telemetry.Histogram = (function () {
    function Histogram(buckets, values, kind, submissions, sum,
      ...
      this.count = this.values.reduce(function (previous, count) {
    ...
    Histogram.prototype.mean = function () {
      var buckets = this.buckets.concat([this.lastBucketUpper(this.buckets,
        this.kind)]);
      var totalHits = 0,
        bucketHits = 0;
      var linearTerm = (buckets[buckets.length - 1] - buckets[buckets.length -
        2]) / 2;
      var exponentialFactor = Math.sqrt(buckets[buckets.length - 1] /
        buckets[buckets.length - 2]);
      var useLinearBuckets = this.kind === "linear" || this.kind ===
        "flag" || this.kind === "boolean" || this.kind === "enumerated";
      this.values.forEach(function (count, i) {
        totalHits += count;
        var centralX = useLinearBuckets ? buckets[i] + linearTerm :
    ...
    Histogram.prototype.percentile = function (percentile) {
      ...
      var linearTerm = buckets[buckets.length - 1] - buckets[buckets.length -
        2];
      var exponentialFactor = buckets[buckets.length - 1] / buckets[
        buckets.length - 2];

      var hitsAtPercentileInBar = this.values.reduce(function (previous,
        count) {
        return previous + count;
      }, 0) * (percentile / 100);
      ...
      var ratioInBar = hitsAtPercentileInBar / this.values[
  ...
  Telemetry.Evolution = (function () {
    ...
    Evolution.prototype.dates = function () {
      return this.data.map(function (entry) {
          ...
          var YYYY = entry.date.substring(0, 4),
    ...
    Evolution.prototype.combine = function (otherEvolution) {
      ...
      var data = Object.keys(dateMap)
        .sort()
        .map(function (date) {
          var entries = dateMap[date];
          var histogram = entries[0].histogram.map(function (count) {
            return 0;
          });
          ...
          return {
            date: date,
            count: entries.reduce(function (previous, entry) {
              return previous + entry.count
            }, 0),
            sum: entries.reduce(function (previous, entry) {
    ...
    Evolution.prototype.histogram = function () {
      var submissions = this.data.reduce(function (submissions, entry) {
        return submissions + entry.count;
      }, 0);
      var sum = this.data.reduce(function (sum, entry) {
    ...
    Evolution.prototype.map = function (callback) {
      ...
      return this.data.sort(function (a, b) {
        ...
        .map(function (entry, i) {
          var histogram = new Telemetry.Histogram(evolution.buckets,
    ...
    Evolution.prototype.means = function () {
      return this.map(function (histogram, i) {
  ...
  Telemetry.getJSON = function (url, callback) {
    ...
    if (Telemetry.CACHE[url] !== undefined) {
      if (Telemetry.CACHE[url] !== null && Telemetry.CACHE[url]._loading) { // Requested but not yet loaded
        ...
        var originalLoadCallback = xhr.onload,
    ...
    Telemetry.getJSON(Telemetry.BASE_URL +
      ...
      function (channels) {
        ...
        channels.forEach(function (channel, i) {
          var versionBuildIds = Telemetry.CHANNEL_VERSION_BUILDIDS[
            channel] = {};
          Telemetry.getJSON(Telemetry.BASE_URL +
  ...
  Telemetry.getEvolution = function Telemetry_getEvolution(channel, version,
    ...
    assert(typeof callback === "function", "`callback` must be a function");
    var dates = (useSubmissionDate ? Telemetry.CHANNEL_VERSION_DATES :
  ...
  Telemetry.getFilterOptions = function Telemetry_getFilterOptions(channel,
    ...
    assert(typeof version === "string", "`version` must be a string");
    assert(typeof callback === "function", "`callback` must be a function");
    var url = Telemetry.BASE_URL + "filters/?channel=" + encodeURIComponent(
  ...
  Telemetry.getVersions = function Telemetry_getVersions(fromVersion,
    ...
    versions.sort();
    return fromVersion !== undefined ? versions.filter(function (version) {
